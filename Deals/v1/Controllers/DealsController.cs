...
namespace Crm.Apps.Deals.V1.Controllers
{
    ...
    public class DealsController : AllowingCheckControllerBase
    {
        ...
        {
            var deal = await _dealsService.GetAsync(id, false, ct);
            if (deal == null)
            {
                return NotFound(id);
            }

            return ReturnIfAllowed(deal, Roles.Deals, deal.AccountId);
        }
        ...
        public async Task<ActionResult<List<Deal>>> GetList([Required] List<Guid> ids, CancellationToken ct = default)
        {
            ...
            return ReturnIfAllowed(deals, Roles.Deals, deals.Select(x => x.AccountId));
        }
            ...
            CancellationToken ct = default)
        {
            ...
            return ReturnIfAllowed(response, Roles.Deals, response.Deals.Select(x => x.AccountId));
        }
        ...
        {
            var oldDeal = await _dealsService.GetAsync(deal.Id, true, ct);
            if (oldDeal == null)
            {
