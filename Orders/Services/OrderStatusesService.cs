...
namespace Crm.Apps.Orders.Services
{
    public class OrderStatusesService : IOrderStatusesService
    {
        ...
        public OrderStatusesService(OrdersStorage storage)
        {
            _storage = storage;
        }

        public Task<OrderStatus> GetAsync(Guid id, bool isTrackChanges, CancellationToken ct)
        {
            return _storage.OrderStatuses
                .AsTracking(isTrackChanges ? QueryTrackingBehavior.TrackAll : QueryTrackingBehavior.NoTracking)
                .FirstOrDefaultAsync(x => x.Id == id, ct);
        }

        public Task<List<OrderStatus>> GetListAsync(IEnumerable<Guid> ids, CancellationToken ct)
        {
            return _storage.OrderStatuses
                .AsNoTracking()
        ...
        public async Task<OrderStatusGetPagedListResponse> GetPagedListAsync(
            Guid accountId,
            OrderStatusGetPagedListRequest request,
            CancellationToken ct)
        {
            var statuses = _storage.OrderStatuses
                .AsNoTracking()
                .Where(x =>
            ...
            return new OrderStatusGetPagedListResponse
            {
        ...
        {
            var changes = new List<OrderStatusChange>();

            await _storage.OrderStatuses
                .Where(x => ids.Contains(x.Id))
