...
namespace Crm.Apps.Orders.Services
{
    public class OrderAttributesService : IOrderAttributesService
    {
        ...
        public Task<OrderAttribute> GetAsync(Guid id, bool isTrackChanges, CancellationToken ct)
        {
            return _storage.OrderAttributes
                .AsTracking(isTrackChanges ? QueryTrackingBehavior.TrackAll : QueryTrackingBehavior.NoTracking)
                .FirstOrDefaultAsync(x => x.Id == id, ct);
        }

        public Task<List<OrderAttribute>> GetListAsync(IEnumerable<Guid> ids, CancellationToken ct)
        {
            return _storage.OrderAttributes
                .AsNoTracking()
        ...
        public async Task<OrderAttributeGetPagedListResponse> GetPagedListAsync(
            Guid accountId,
            OrderAttributeGetPagedListRequest request,
            CancellationToken ct)
        {
            var attributes = _storage.OrderAttributes
                .AsNoTracking()
                .Where(x =>
            ...
            return new OrderAttributeGetPagedListResponse
            {
        ...
        {
            var changes = new List<OrderAttributeChange>();

            await _storage.OrderAttributes
                .Where(x => ids.Contains(x.Id))
