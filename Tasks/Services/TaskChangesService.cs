...
namespace Crm.Apps.Tasks.Services
{
    public class TaskChangesService : ITaskChangesService
    {
        ...
        public TaskChangesService(TasksStorage storage)
        {
            _storage = storage;
        }

        public async Task<TaskChangeGetPagedListResponse> GetPagedListAsync(
            TaskChangeGetPagedListRequest request,
            CancellationToken ct)
        {
            var changes = _storage.TaskChanges
                .AsNoTracking()
                .Where(x =>
                    (request.TaskId.IsEmpty() || x.TaskId == request.TaskId) &&
                    (!request.MinCreateDate.HasValue || x.CreateDateTime >= request.MinCreateDate) &&
                    (!request.MaxCreateDate.HasValue || x.CreateDateTime <= request.MaxCreateDate));

            return new TaskChangeGetPagedListResponse
            {
