...
namespace Crm.Apps.Areas.Products.Controllers
{
    ...
    public class ProductStatusesController : UserContextController
    {
        ...
        public async Task<ActionResult<ProductStatus>> Get([Required] Guid id, CancellationToken ct = default)
        {
            ...
            return ReturnIfAllowed(status, new[] {Role.AccountOwning, Role.ProductsManagement}, status.AccountId);
        }
            ...
            CancellationToken ct = default)
        {
            ...
            var statuses = await _productStatusesService.GetPagedListAsync(parameter, ct);
