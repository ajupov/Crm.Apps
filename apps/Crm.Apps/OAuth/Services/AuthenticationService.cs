...
namespace Crm.Apps.OAuth.Services
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public AuthenticationProperties GetExternalProperties(
            string state,
            string ipAddress,
            string callbackUri)
        {
            _hotStorage.SetValue(state, ipAddress, TimeSpan.FromMinutes(10));
        ...
        {
            var savedIpAddress = _hotStorage.GetValue<string>(request.State);
            if (clientIpAddress != savedIpAddress)
            {
                return new CallbackResponse(AuthenticationResponseError.IpAddressSubstitution);
            }
        ...
        private async Task<IdentityType> GetIdentityTypeAsync(
        ...
        private static UserGender GetGender(
            string gender)
        {
