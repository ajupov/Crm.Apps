...
namespace Crm.Apps.Activities.Controllers
{
    ...
    public class ActivityAttributesController : ControllerBase
    {
        ...
        public async Task<ActionResult<ActivityAttribute>> Get(Guid id, CancellationToken ct = default)
        {
            ...
            var attribute = await _activityAttributesService.GetAsync(id, ct).ConfigureAwait(false);
            if (attribute == null)
            {
                return NotFound();
            }
        ...
        public async Task<ActionResult<List<ActivityAttribute>>> GetList(List<Guid> ids, CancellationToken ct = default)
        {
            ...
            var attributes = await _activityAttributesService.GetListAsync(ids, ct).ConfigureAwait(false);
        ...
        {
            var attributes = await _activityAttributesService.GetPagedListAsync(parameter, ct).ConfigureAwait(false);
        ...
        public async Task<ActionResult> Update(ActivityAttribute attribute, CancellationToken ct = default)
        {
            ...
            var oldAttribute = await _activityAttributesService.GetAsync(attribute.Id, ct).ConfigureAwait(false);
